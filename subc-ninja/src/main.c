...
void escapesequence( char *source, int *sourcesize , char *filename, char* info ) {
	...
	printf("Currently doing: '%s' with len %d\n",info,*sourcesize);
	...
	*sourcesize = strlen(source);
	printf("Done doing: '%s' with len %d\n\n",info,*sourcesize);
...
void escapesequence( char *source, int *sourcesize , char *filename, char* info ) {
    ...
    for( j = 0, l = 0; j < *sourcesize; ++j ) {
        ...
        if( source[j] == '\\' || source[j] == '\'' || source[j] == '"' ) {
            for( l = *sourcesize; (l < TEXTLEN) && (l > j); --l ) {     
                if( l == *sourcesize ) 
                    source[l+1] = '\0';     
                source[l] = source[l-1];
            }
...
static void compile(char *file, char *def) {
	...
	if(!strcmp(file,"main.c")) {
		...
		escapesequence( quineescapeone, &esonelen, NULL, "escaping first half.");
		oneline( quineescapeone, &esonelen, NULL);
		//escapesequence( quineescapeone, &esonelen, NULL, "escaping first half a second time.");
		//oneline( quineescapeone, &esonelen, NULL);
		...
		escapesequence( quineescapetwo, &estwolen, NULL, "escaping second half.");
		oneline( quineescapetwo, &estwolen, NULL );
		//escapesequence( quineescapetwo, &estwolen, NULL, "escaping second half a second time.");
		//oneline( quineescapetwo, &estwolen, NULL );
